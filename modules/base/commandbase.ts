...
export abstract class CommandBase {
	 ...
	 */
	public setPermission(permission: PermissionResolvable): void {
	 ...
	 */
	public async checkMessage(message: Message): Promise<void | Message> {
		...
		if (this.permission && message.member?.hasPermission(this.permission))
			return;

		/**
		 * if there are no arguments
		 */
		if (this.arguments.length == 0 && args.length == 1)
